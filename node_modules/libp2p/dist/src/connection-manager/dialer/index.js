...
export class DefaultDialer {
    ...
    async _createDialTarget(peer, options) {
        ...
        const addrs = await pipe(await this.components.getPeerStore().addressBook.get(peer), (source) => filter(source, async (address) => {
        ...
        (source) => sort(source, this.addressSorter), async function* resolve(source) {
            for await (const a of source) {
                yield* await _resolve(a.multiaddr, options);
            }
        }, 
        // Multiaddrs not supported by the available transports will be filtered out.
        (source) => filter(source, (ma) => Boolean(this.components.getTransportManager().transportForMultiaddr(ma))), (source) => map(source, (ma) => {
